...
An **API document** served at `/api-docs` without a credential, the contract the service builds
and nothing else, so an integrator reads what the service serves and the two cannot differ.
